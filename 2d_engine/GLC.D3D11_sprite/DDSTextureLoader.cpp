...
#undef ISBITMASK
        ...
        _Out_writes_(mipCount*arraySize) D3D11_SUBRESOURCE_DATA* initData) noexcept
    {
        ...
        for (size_t j = 0; j < arraySize; j++)
        {
            ...
            {
                HRESULT hr = GetSurfaceInfo(w, h, format, &NumBytes, &RowBytes, {});
                if (FAILED(hr))
                    return hr;
        ...
        _Outptr_opt_ ID3D11ShaderResourceView** textureView) noexcept
    {
        ...
        if (autogen)
        {
            ...
            if (SUCCEEDED(hr))
            {
                size_t numBytes = 0;
                size_t rowBytes = 0;
                hr = GetSurfaceInfo(width, height, format, &numBytes, &rowBytes, {});
                if (FAILED(hr))
                    return hr;
                ...
                if (arraySize > 1)
                {
                    ...
                    for (UINT item = 0; item < arraySize; ++item)
                    {
                        ...
                        d3dContext->UpdateSubresource(tex, res, {}, pSrcBits, static_cast<UINT>(rowBytes), static_cast<UINT>(numBytes));
                        pSrcBits += numBytes;
                    }
                }
                else
                {
                    d3dContext->UpdateSubresource(tex, 0, {}, bitData, static_cast<UINT>(rowBytes), static_cast<UINT>(numBytes));
                }
