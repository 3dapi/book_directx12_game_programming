...
bool BlurApp::Initialize()
{
	...
	mBlurFilter = std::make_unique<BlurFilter>(m_d3dDevice.Get(), 
...
void BlurApp::Draw(const GameTimer& gt)
{
	...
	mBlurFilter->Execute(m_d3dCommandList.Get(), mPostProcessRootSignature.Get(), 
	...
	m_d3dCommandList->CopyResource(CurrentBackBuffer(), mBlurFilter->Output());
...
void BlurApp::BuildDescriptorHeaps()
{
	...
	ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&mCbvSrvUavDescriptorHeap)));
...
void BlurApp::BuildPSOs()
{
	...
	ThrowIfFailed(m_d3dDevice->CreateComputePipelineState(&horzBlurPSO, IID_PPV_ARGS(&mPSOs["horzBlur"])));
	...
	ThrowIfFailed(m_d3dDevice->CreateComputePipelineState(&vertBlurPSO, IID_PPV_ARGS(&mPSOs["vertBlur"])));
}
