...
void InstancingAndCullingApp::Draw(const GameTimer& gt)
{
	...
	m_d3dCommandList->SetGraphicsRootShaderResourceView(1, matBuffer->GetGPUVirtualAddress());
	...
	m_d3dCommandList->SetGraphicsRootDescriptorTable(3, mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());

    DrawRenderItems(m_d3dCommandList.Get(), mOpaqueRitems);
...
void InstancingAndCullingApp::UpdateInstanceData(const GameTimer& gt)
{
	...
	auto currInstanceBuffer = m_frameRscCur->InstanceBuffer.get();
...
void InstancingAndCullingApp::LoadTextures()
{
	...
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(m_d3dDevice.Get(),
		m_d3dCommandList.Get(), stoneTex->Filename.c_str(),
	...
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(m_d3dDevice.Get(),
		m_d3dCommandList.Get(), tileTex->Filename.c_str(),
	...
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(m_d3dDevice.Get(),
		m_d3dCommandList.Get(), crateTex->Filename.c_str(),
	...
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(m_d3dDevice.Get(),
		m_d3dCommandList.Get(), defaultTex->Filename.c_str(),
...
void InstancingAndCullingApp::BuildDescriptorHeaps()
{
	...
	m_d3dDevice->CreateShaderResourceView(stoneTex.Get(), &srvDesc, hDescriptor);
	...
	m_d3dDevice->CreateShaderResourceView(defaultTex.Get(), &srvDesc, hDescriptor);
}
...
void InstancingAndCullingApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    ...
    for(size_t i = 0; i < ritems.size(); ++i)
    {
		...
		auto instanceBuffer = m_frameRscCur->InstanceBuffer->Resource();
		m_d3dCommandList->SetGraphicsRootShaderResourceView(0, instanceBuffer->GetGPUVirtualAddress());
