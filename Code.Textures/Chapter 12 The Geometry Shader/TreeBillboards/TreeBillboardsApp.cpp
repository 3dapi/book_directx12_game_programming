...
void TreeBillboardsApp::Draw(const GameTimer& gt)
{
	...
	m_d3dCommandList->SetPipelineState(mPSOs["treeSprites"].Get());
	DrawRenderItems(m_d3dCommandList.Get(), mRitemLayer[(int)RenderLayer::AlphaTestedTreeSprites]);
...
void TreeBillboardsApp::LoadTextures()
{
	...
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(m_d3dDevice.Get(),
		m_d3dCommandList.Get(), treeArrayTex->Filename.c_str(),
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	ThrowIfFailed(m_d3dDevice->CreateGraphicsPipelineState(&treeSpritePsoDesc, IID_PPV_ARGS(&mPSOs["treeSprites"])));
}
...
void TreeBillboardsApp::BuildFrameResources()
{
    ...
    {
        m_frameRscLst.push_back(std::make_unique<FrameResource>(m_d3dDevice.Get(),
